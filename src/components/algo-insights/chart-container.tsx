...
const generateTradeLog = (
    ...
    const nextCandleStart = tool.entryDate.getTime() + getTimeframeMinutes(timeframe) * 60 * 1000;
    const entryIndex = priceData.findIndex(p => p.date.getTime() >= nextCandleStart);
...
export function ChartContainer({ tab }: { tab: 'backtester' | 'journal' }) {
  ...
  const hasVolumeData = priceData.some((point) => typeof point.volume === 'number' && !isNaN(point.volume));
  ...
  useEffect(() => {
    ...
    if (savedSettingsRaw) {
        try {
            ...
            if (typeof savedSettings.showVolumeBars === 'boolean') setShowVolumeBars(savedSettings.showVolumeBars);
        } catch (e) {
  ...
  useEffect(() => {
    const settings = { timeZone, sessionStartTime, pipValue, openingRangeDuration, emaPeriod, showVolumeBars };
    localStorage.setItem(APP_SETTINGS_KEY, JSON.stringify(settings));
  }, [timeZone, sessionStartTime, pipValue, openingRangeDuration, emaPeriod, showVolumeBars]);
    ...
    reader.onload = (e) => {
        try {
            ...
            const parsedData: PriceData[] = dataRows.map((row, index) => {
                ...
                const localTime = columns[0];
                const openStr = columns[1];
                const highStr = columns[2];
                const lowStr = columns[3];
                const closeStr = columns[4];
                const volumeStr = columns[5];
                ...
                return { date, open, high, low, close, volume: isNaN(volume ?? NaN) ? undefined : volume, wick: [low, high] };
       ...
       <AlertDialog open={showRestoreDialog} onOpenChange={setShowRestoreDialog}>
        ...
        <div className="absolute inset-0">
            <InteractiveChart
                ...
                emaPeriod={emaPeriod}
                showVolumeBars={showVolumeBars && hasVolumeData}
                        ...
                        {hasVolumeData && (
                            <div className="flex items-center justify-between rounded-lg border px-3 py-2">
                                <div className="space-y-0.5">
                                    <Label htmlFor="volume-bars" className="text-sm font-medium">Volume bars</Label>
                                    <p className="text-xs text-muted-foreground">Show/hide volume on the chart</p>
                                </div>
                                <Switch id="volume-bars" checked={showVolumeBars} onCheckedChange={setShowVolumeBars} />
                            </div>
                        )}
                    </div>
                </PopoverContent>
