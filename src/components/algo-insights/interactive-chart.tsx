  ...
  const aggregatedData = useMemo(() => {
    ...
    for (const point of filteredByDate) {
      ...
      if (!currentCandle || currentCandle.date.getTime() !== bucketTimestamp) {
        ...
        currentCandle.volume = (currentCandle.volume ?? 0) + (typeof point.volume === 'number' ? point.volume : 0);
        ...
        >
          ...
          {showVolumeBars && hasVolumeData && (
            <YAxis
              ...
              tickLine={false}
              axisLine={false}
              tickFormatter={(value) => typeof value === 'number' ? `${Math.round(value / 1000)}k` : ''}
              width={42}
              domain={[0, 'auto']}
              allowDataOverflow={true}
            />
